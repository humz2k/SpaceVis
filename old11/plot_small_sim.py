...
SpaceVis.gpurenderer.GPU_loadpoints(data[-1])
...
for cam_z in tqdm.tqdm(np.linspace(-10,32+10,600)):
    ...
    out = SpaceVis.gpurenderer.draw_frame_GPU(cam_pos,cam_rot,e,im_x,im_y,data[-1],10,32,4,4,4)
    #print(np.max(out))

    out.tofile("frames/small" + str(idx) + ".dat")
